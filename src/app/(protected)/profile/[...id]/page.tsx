...
function ProfilePage({ params }: { params: { id: string } }) {
  ...
  const CloseImage = () => {
    handleClosePostModal(removeCommentsRedux());
  };
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        const posts = userResponse?.posts?.sort(
          (a: any, b: any) => b.id - a.id
        );
        dispatch(getProfilePosts(posts));

        parseInt(params.id) === userLogged?.id &&
          dispatch(deleteAllPostRedux());
      } catch (error) {
        console.log(error);
        ...
        {user === null && showNotFound && (
          <div className="flex-h-screen justify-center mt-16">
            <h1 className="text-3xl font-bold">
              No user found with this ID: {params.id}
            </h1>
          </div>
        )}
      </AnimatedWrapper>
